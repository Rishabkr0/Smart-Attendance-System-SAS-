...
import { Camera, CheckCircle, AlertCircle, Trash2, ShieldCheck, Fingerprint, ScanEye, User, ArrowLeft } from 'lucide-react';
...
const SelfEnrollment = () => {
    ...
    useEffect(() => {
        const loadModels = async () => {
            try {
                ...
                setStatus({ message: `AI Model Error: ${err.message || 'Check network connection'}`, type: 'error' });
            }
        };
        loadModels();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await api.post('/students', { 
                ...
                course: formData.department, // Map department to course as per model
                faceDescriptors 
            });
    ...
    return (
        <div className="min-h-screen flex flex-col bg-background relative overflow-hidden font-body">
            ...
            <div className="absolute top-0 left-0 w-[600px] h-[600px] bg-primary/5 blur-[150px] -z-10 rounded-full -translate-x-1/3 -translate-y-1/3 animate-pulse-slow"></div>
            <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-tertiary/5 blur-[120px] -z-10 rounded-full translate-x-1/4 translate-y-1/4"></div>

            <header className="p-8 relative z-20 flex justify-between items-center bg-background/50 backdrop-blur-md">
                <div className="flex items-center gap-3">
                    <ShieldCheck size={32} className="text-primary" />
                    <span className="font-bold text-2xl font-display tracking-tight text-on-surface">Sentinel AI</span>
                </div>
                <Link to="/login" className="flex items-center gap-2 text-primary font-bold hover:underline underline-offset-4 text-sm group">
                    <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Back to Portal
                </Link>
            </header>

            <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-12 z-10 fade-in">
                <div className="max-w-6xl mx-auto space-y-8">
                    <header className="space-y-2 text-center max-w-2xl mx-auto mb-12">
                        <h1 className="text-4xl md:text-5xl font-extrabold font-display leading-tight text-on-surface">Student Portal <br/><span className="text-primary">Self-Enrollment</span></h1>
                        <p className="text-on-surface-variant font-medium text-lg">Synthesize your biometric data and official credentials to access the automated attendance system.</p>
                    </header>
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            <form onSubmit={handleSubmit} className="sentinel-card space-y-10 relative overflow-hidden">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-on-surface-variant/70 px-1">Course / Department</label>
                                        <input 
                        ...
                        <div className="lg:col-span-3 space-y-6">
                            <div className="sentinel-card !p-0 overflow-hidden aspect-video relative group border-4 border-white shadow-2xl">
                                ...
                                <div className="absolute inset-0 z-10 pointer-events-none">
                                    ...
                                    <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-center opacity-20">
                                         <div className="w-64 h-80 border-2 border-white/30 rounded-[100px] border-dashed shadow-[0_0_50px_rgba(255,255,255,0.2)]"></div>
                                    </div>
                                </div>
                                ...
                                <div className="absolute bottom-8 inset-x-8 z-20 flex items-center justify-between">
                                     <div className="sentinel-glass rounded-2xl px-6 py-3 flex items-center gap-3">
                                        ...
                                        <span className="text-xs font-bold font-display uppercase tracking-widest text-on-surface">Samples: {faceDescriptors.length} / 5</span>
                                     </div>
                                     <button 
                                        onClick={captureFace} disabled={!isModelLoaded || faceDescriptors.length >= 5}
                                        className={`p-6 rounded-3xl shadow-2xl transition-all group pointer-events-auto ${faceDescriptors.length >= 5 ? 'bg-gray-400 cursor-not-allowed text-white/50' : 'bg-primary hover:bg-primary-container text-white shadow-primary/40 active:scale-95'}`}
                                     >
                                        <Camera size={28} className={faceDescriptors.length >= 5 ? '' : 'group-hover:scale-110 transition-transform'} />
                                     </button>
                                </div>
                            </div>
                            ...
                            <div className="sentinel-card flex flex-col gap-6">
                                <div className="flex items-center justify-between">
                                    ...
                                    <button onClick={() => setFaceDescriptors([])} className="text-red-500 p-2 hover:bg-red-50 rounded-xl transition-all" title="Clear all samples">
                                        <Trash2 size={18} />
                                    </button>
                                </div>
                                ...
                                <div className="bg-indigo-50 border border-indigo-100 p-6 rounded-2xl">
                                    <p className="text-xs font-semibold text-indigo-700 leading-relaxed italic">
                                        **Sentinel Note:** Ensure your face is well-lit. Capture exactly 5 variants (straight, slight left, slight right, up, down) for maximum recognition precision.
                                    </p>
                                </div>
                            </div>
                        </div>
